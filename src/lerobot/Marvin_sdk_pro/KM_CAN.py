...
class Motor:
    ...
    def recv_data(self, q: float, dq: float, tau: float, err: int, err_string: str,temp_mos:int,temp_motor:int):
        self.state_q = q
        self.state_dq = dq
        self.state_tau = tau
        self.err = err
        self.err_str = err_string
    ...
    def getPosition(self):
        ...
        """
        # print("getPosition:", self.state_q)
        return self.state_q
...
class KMGripperControl():
    ...
    def __init__(self, serial_device=None, robot: Marvin_Robot = None):
        """
        ...
        # 添加CAN ID到电机对象的映射
        self.canid_to_motor = dict()  # 新的映射表：CAN ID -> Motor对象
        # 明确左右手映射

        self.data_save = bytes()  # save data
        self.data_send = dict()  # save data to be sent
        self.robot = robot
        self.robot_com = 1
        self.left_ch = set()
        self.right_ch = set()
    ...
    def controlMIT(self, KM_Motor, kp: float, kd: float, q: float, dq: float, tau: float):
        """
        MIT Control Mode Function 达妙电机MIT控制模式函数
        :param KM_Motor: Motor object 电机对象
        :param kp: kp
        :param kd:  kd
        ...
        :return: None
        """
        if KM_Motor.SlaveID not in self.motors_map:
            print("controlMIT ERROR : Motor ID not found")
            return
        kp_uint = float_to_uint(kp, 0, 500, 12)
        kd_uint = float_to_uint(kd, 0, 5, 12)
        MotorType = KM_Motor.MotorType
        ...
        self.__send_data_can(KM_Motor.SlaveID, data_buf)
        # self.recv()  # receive the data from serial port
        
        #if self.err not None:
        # print("err")
        # print(KM_Motor.err)
    
    def control_delay(self, KM_Motor, kp: float, kd: float, q: float, dq: float, tau: float, delay: float):
        """
        ...
        """
        self.controlMIT(KM_Motor, kp, kd, q, dq, tau)
        sleep(delay)
    ...
    def control_Pos_Vel(self, Motor, P_desired: float, V_desired: float):
        ...
        self.__send_data_can(motorid, data_buf)
        # time.sleep(0.001)
        # self.recv()  # receive the data from serial port
    ...
    def enable_old(self, Motor ,ControlMode):
        ...
        self.__send_data_can(enable_id, data_buf)
        sleep(0.1)
        # self.recv()  # receive the data from serial port
    ...
    def recv(self):
        if self.robot is None:
            return
        for arm in ("A", "B"):
            tag, receive_hex_data = self.robot.get_485_data(arm, 1)
            if tag <= 0 or not receive_hex_data:
                continue
            try:
                hex_parts = receive_hex_data.split()
                data_bytes = bytes(int(b, 16) for b in hex_parts[:tag])
            except ValueError:
                continue

            if len(data_bytes) >= 12:
                can_id = int.from_bytes(data_bytes[0:4], byteorder='little', signed=False)
                packet_data = data_bytes[4:12]
            elif len(data_bytes) >= 8:
                can_id = 0
                packet_data = data_bytes[:8]
            else:
                continue

            self.__process_packet(packet_data, can_id, 0x11)
        ...
        if CMD == 0x11:
            if CANID != 0x00:
                if CANID in self.motors_map:
                    # err = np.uint16((np.uint16(data[0]) << 8))
                    error_code = (data[0] & 0xF0) >> 4  # 取高4位，得到错误码
                    error_msg = ERROR_MESSAGES.get(error_code, f"未知故障(0x{error_code:X})")
                    ...
                    self.motors_map[CANID].recv_data(recv_q, recv_dq, recv_tau,error_code, error_msg, temp_mos,temp_rotor)
            else:
                ...
                if MasterID in self.motors_map:
                    ...
                    recv_dq = uint_to_float(dq_uint, -DQ_MAX, DQ_MAX, 12)
                    recv_tau = uint_to_float(tau_uint, -TAU_MAX, TAU_MAX, 12)
                    self.motors_map[MasterID].recv_data(recv_q, recv_dq, recv_tau,error_code,error_msg, temp_mos,temp_rotor)
    ...
    def add_to_ch(self, Motor, ch: str):
        """
        add motor to the specified channel 添加电机到指定通道
        :param Motor: Motor object 电机对象
        :param ch: Channel name 通道名称 left or right
        """
        if ch == "left":
            if not hasattr(self, 'left_ch'):
                self.left_ch = set()
            self.left_ch.add(Motor.SlaveID)

        elif ch == "right":
            if not hasattr(self, 'right_ch'):
                self.right_ch = set()
    ...
    def __send_data_can(self, motor_id, data):
        """
        Send data to motor via CAN socket
        :param sock: socket object (sock0 or sock1)
        :param motor_id: int, motor ID
        :param data: bytes, up to 8 bytes for CAN frame
        """
        ...
        if motor_id in self.left_ch:
            ...
            # 广播/配置命令(如 0x7FF)，从 data 首字节提取 SlaveID 判断通道
            slave_id = int(data[0])
            if slave_id in self.left_ch:
                target_arm = 'A'
            elif slave_id in self.right_ch:
                target_arm = 'B'
            else:
                print(f"[CAN] Error: cannot determine arm for motor_id=0x{motor_id:X}")
                return

        can_id_bytes = int(motor_id).to_bytes(4, byteorder='little', signed=False)
        data_bytes = data.astype(np.uint8).tobytes().ljust(8, b'\x00')
        payload = can_id_bytes + data_bytes
        hex_data = ' '.join(f"{b:02X}" for b in payload)
        try:
            success, _ = self.robot.set_485_data(target_arm, hex_data, len(hex_data),1)
            if not success:
                print("[CAN] Error sending via 485: set_485_data failed")
        except Exception as e:
            print(f"[CAN] Error sending via 485: {e}")
        return
    ...
    def __read_RID_param(self, Motor, RID):
        ...
        self.__send_data_can(0x7FF, data_buf)
    ...
    def switchControlMode(self, Motor, ControlMode):
        ...
        # 通过机器人 485 接口时没有 serial_，无法用 recv_set_param_data 验证
        if not hasattr(self, 'serial_') or self.serial_ is None:
            sleep(0.1)
            return True
