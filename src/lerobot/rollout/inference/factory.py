...
from .chunk import ChunkInferenceEngine
...
@InferenceEngineConfig.register_subclass("chunk")
@dataclass
class ChunkInferenceConfig(InferenceEngineConfig):
    """Open-loop chunk execution: one inference, one whole-chunk HTTP send.

    Every ``chunk_interval_s`` seconds the policy runs once and the first
    ``n_action_steps`` actions of the produced chunk are dispatched to the
    robot in a single send (via ``send_action_chunk``).  The robot executes
    the chunk open-loop until the next chunk arrives.
    """

    # Number of actions from each chunk to dispatch open-loop.
    n_action_steps: int = 25
    # Seconds to wait between successive chunk sends (open-loop hold time).
...
def create_inference_engine(
    ...
    *,
    ...
    if isinstance(config, ChunkInferenceConfig):
        return ChunkInferenceEngine(
            policy=policy,
            preprocessor=preprocessor,
            postprocessor=postprocessor,
            dataset_features=dataset_features,
            ...
            chunk_interval_s=config.chunk_interval_s,
        )
