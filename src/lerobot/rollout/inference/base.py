...
class InferenceEngine(abc.ABC):
    ...
    def get_action_chunk(self, obs_frame: dict | None) -> torch.Tensor | None:
        """Return a whole action chunk ``[N, A]`` to dispatch at once, or ``None``.

        Only meaningful for backends whose :attr:`produces_chunks` is True.
        Default: no chunk available.
        """
        return None
    ...
    def produces_chunks(self) -> bool:
        """True if this backend dispatches whole action chunks via ``get_action_chunk``.

        When True, rollout strategies route through ``send_next_action_chunk``
        (one HTTP send per chunk) instead of the per-tick ``send_next_action``.
        """
        return False
