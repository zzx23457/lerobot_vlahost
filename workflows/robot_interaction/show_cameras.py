...
def _grab_mjpeg_frame(session: requests.Session, base_url: str, stream_url: str, timeout: float = 2.0) -> np.ndarray:
    """从 MJPEG 流拉单帧 JPEG，解码为 RGB (H, W, 3)。

    与 MarvainM6HttpRobot._grab_mjpeg_frame 逻辑一致；这里复制一份是因为该
    方法是实例方法，show_cameras 不持有 robot 实例。
    """
    if not stream_url.startswith("http"):
        stream_url = f"{base_url.rstrip('/')}/{stream_url.lstrip('/')}"
    ...
    try:
        while True:
            try:
                ...
                if quad_field is not None:
                    if "data" in quad_field:
                        # 旧格式：base64 内嵌帧
                        quad_rgb = MarvainM6HttpRobot._decode_image(quad_field["data"])
                    elif "stream_url" in quad_field:
                        # 新格式：从 MJPEG 流拉单帧
                        quad_rgb = _grab_mjpeg_frame(session, http_url, quad_field["stream_url"])
                    else:
                        quad_rgb = None
                    if quad_rgb is None:
                        # 服务端 quad_image 字段不可识别；这一帧跳过
                        raise RuntimeError("quad_image has neither 'data' nor 'stream_url'")
                    if args.show_quad:
                        cv2.imshow(
                            "quad_image (raw)",
