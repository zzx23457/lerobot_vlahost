...
>>> OLD 备份：本会话开始时仓库里的版本。
>>> 与 ../arm_control_http.py 当前版本的差异：
>>>   1. get_current_state() 直接 data["joint_states"]["positions"]（不防御 None）
>>>   2. get_current_state() gripper 用 list[0] 索引（不兼容 dict 格式）
>>>   3. send_action() payload 字段名 joint_left/right（被服务端静默丢弃）
>>>
>>> 回滚：cp old/arm_control_http.py ../arm_control_http.py
...
class HTTPArmController:
    ...
    def get_current_state(self):
        ...
        try:
            ...
            # 臂关节（弧度）
            arm_joints_rad = data["joint_states"]["positions"]
            arm_joints_deg = np.degrees(arm_joints_rad).tolist()

            # 夹爪（弧度）
            gripper_left_rad = data.get("gripper_left", [0.0])[0]
            gripper_right_rad = data.get("gripper_right", [0.0])[0]
            gripper_left_deg = np.degrees(gripper_left_rad)
            gripper_right_deg = np.degrees(gripper_right_rad)

            return {
                'arm_joints': arm_joints_deg,  # 14个
                'gripper_left': gripper_left_deg,
                'gripper_right': gripper_right_deg,
                'all_joints': arm_joints_deg + [gripper_left_deg, gripper_right_deg]  # 16个
            }
