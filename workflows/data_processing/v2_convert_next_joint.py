...
**VARIANT**: This version uses the NEXT timestep's joint_pos as arm_cmd instead of v1_actions[:, 14:28].

支持两种调用模式:
1. **legacy 模式** (无 CLI): 读取同目录 `v2_convert_config.json` 处理全部声明数据集。
   - 旧调用: `python3 workflows/v2_convert_next_joint.py`
2. **CLI 单任务模式** (新增, 非交互): 由 UI 或脚本驱动。
   - `python3 workflows/v2_convert_next_joint.py --dataset-root <v1_path> --output-root <v2_path> [--v2-suffix _v2] [--camera-enabled 0,1,1,1] [--dry-run]`
...
def convert_dataset(v1_root: Path, v2_root: Path, disabled_cameras: set[str] | None = None) -> None:
    ...
    print(f"      VARIANT: arm_cmd uses NEXT timestep's joint_pos (v1_actions[:, 42:56])")
    ...
    # NEW LOGIC: arm_cmd is the NEXT timestep's joint_pos, not v1_actions[:, 14:28]
    arm_cmd_current = v1_actions[:, 42:56] * RAD_TO_DEG   # (T, 14) - current joint_pos
    arm_cmd    = arm_cmd_current.copy()                    # prepare for shift
    left_next  = left_grip.copy()
    right_next = right_grip.copy()

    # Shift forward by one timestep per episode (same logic as left_next/right_next)
    for ep_id in np.unique(ep_indices):
        mask = ep_indices == ep_id
        for src, dst in [(arm_cmd_current, arm_cmd), (left_grip, left_next), (right_grip, right_next)]:
            ep_slice = src[mask]
            if len(ep_slice) >= 2:
                # shift: [t0, t1, t2, ..., tN] → [t1, t2, ..., tN, tN]
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="v1 → v2 schema conversion (next_joint variant). CLI 单任务模式 (新增) 或 JSON 配置模式 (旧兼容)。",
    )
    parser.add_argument(
        "--dataset-root",
        type=Path,
...
def _parse_camera_enabled_next(raw: str | None) -> list[int] | None:
...
def _resolve_jobs(args: argparse.Namespace) -> list[tuple[Path, Path, set[str]]]:
    ...
    """
    cameras = _parse_camera_enabled_next(args.camera_enabled)
    if cameras is None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    cameras_repr = _parse_camera_enabled_next(args.camera_enabled)
